...
VideoManager::_setActiveVehicle(Vehicle* vehicle)
{
    ...
    if(_activeVehicle) {
        UAVInfoManager *uavInfoManager = qgcApp()->toolbox()->httpAPIManager()->getUAVInfoManager();
        bool isLogin = qgcApp()->toolbox()->httpAPIManager()->getUserManager()->isLogin();
        if (isLogin && uavInfoManager) {
            QString videoStreamUrl = uavInfoManager->getVideoStreamUrl(_activeVehicle->id());
            qDebug() << __FILE__ << __func__ << __LINE__ << videoStreamUrl.contains(QRegExp("^rtsp://", Qt::CaseInsensitive)) << videoStreamUrl;
            if (!videoStreamUrl.contains(QRegExp("^rtsp://", Qt::CaseInsensitive))) videoStreamUrl = "";
            _videoSettings->rtspUrl()->setRawValue(QVariant(videoStreamUrl));
            _videoSettings->videoSource()->setRawValue(QVariant(_videoSettings->rtspVideoSource()));
        }
