...
#endif
{
    ...
    connect(this, &UDPLink::bytesReceived, this, &UDPLink::_onBytesReceived);
    moveToThread(this);
}
...
void UDPLink::_onBytesReceived(LinkInterface *link, QByteArray data)
{
    if (link == qgcApp()->toolbox()->linkManager()->mavlinkForwardingLink().get()) {
        QList<SharedLinkInterfacePtr> links = qgcApp()->toolbox()->linkManager()->links();
        for (int i = 0; i < links.size(); i++) {
            SharedLinkInterfacePtr l = links.at(i);
            if (l.get() != link) {
                l->writeBytesThreadSafe(data.data(), data.length());
            }
        }
    }
}
