...
#endif
//    qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << "write data to tcp start... length=" << data.length();
//    qDebug() << data.toHex();
//    qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << "write data to tcp end---";
    if (_socket) {
        _socket->write(data);
...
void TCPLink::_readBytes()
{
    if (_socket) {
        ...
        //qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << "read data from tcp start---" << byteCount;
        if (byteCount)
        {
            QByteArray buffer;
            ...
            //qDebug() << buffer.toHex();
            //qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << "read data from tcp end---";
...
void TCPConfiguration::setId(const QString id)
{
    _id = id;
}
...
void TCPConfiguration::loadSettings(QSettings& settings, const QString& root)
{
    ...
    _id = settings.value("id", "-1").toString();
    settings.endGroup();
}
