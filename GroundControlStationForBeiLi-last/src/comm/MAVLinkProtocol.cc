...
void MAVLinkProtocol::receiveBytes(LinkInterface* link, QByteArray b)
{
    ...
    for (int position = 0; position < b.size(); position++) {
        if (mavlink_parse_char(mavlinkChannel, static_cast<uint8_t>(b[position]), &_message, &_status)) {
            ...
            } else if (_message.msgid == MAVLINK_MSG_ID_BIND_FLYER_PLANE_ACK) {
                mavlink_bind_flyer_plane_ack_t bind_plane;
                mavlink_msg_bind_flyer_plane_ack_decode(&_message, &bind_plane);
                qDebug() << __FILE__ << __func__ << __LINE__ << "bind ack:" << bind_plane.ack;
            }
