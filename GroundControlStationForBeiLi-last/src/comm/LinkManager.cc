...
void LinkManager::setToolbox(QGCToolbox *toolbox)
{
    ...
    connect(&_timer, &QTimer::timeout, this, &LinkManager::_bindPlane);
}
...
bool LinkManager::createConnectedLink(SharedLinkConfigurationPtr& config, bool isPX4Flow)
{
    ...
    case LinkConfiguration::TypeTcp:
        ...
        connect(link.get(), &LinkInterface::connected, this, &LinkManager::_tcpOrUdpLinkConnected);
        break;
#ifdef QGC_ENABLE_BLUETOOTH
...
void LinkManager::_linkDisconnected(void)
{
    ...
    if (link->linkConfiguration().get()->type() == LinkConfiguration::TypeTcp) {
        disconnect(link, &LinkInterface::connected, this, &LinkManager::_tcpOrUdpLinkConnected);
        for (int i = 0; i < _tcpBindLinks.count(); i++) {
            if (_tcpBindLinks[i].get() == link) {
                _tcpBindLinks.removeAt(i);
        ...
        bool has = false;
        for (int i=0; i<_tcpBindLinks.count(); i++) {
            LinkInterface* tcpLink = _tcpBindLinks[i].get();
            if (tcpLink && tcpLink != link && tcpLink->linkConfiguration()->type() == LinkConfiguration::TypeTcp && tcpLink->isConnected()) {
                has = true;
            }
        }
        if (!has && _timer.isActive()) _timer.stop();
    }
...
void LinkManager::_tcpOrUdpLinkConnected()
{
...
#if 0
    _currentConnectedLink = link;
    QTimer::singleShot(5000, this, &LinkManager::_bindPlane);
#else
    for (int i = 0; i < _rgLinks.count(); i++) {
        if (_rgLinks[i].get() == link) {
            _tcpBindLinks.append(_rgLinks[i]);
            break;
        }
    }
...
void LinkManager::_bindPlane()
{
#if 0
    LinkInterface *link = _currentConnectedLink;
    if (!link || !containsLink(link)) {
        _currentConnectedLink = nullptr;
        return;
    }
    TCPConfiguration* config = dynamic_cast<TCPConfiguration*>(link->linkConfiguration().get());
    if (config && config->id().compare("-1") != 0 && _toolbox->httpAPIManager()->getUserManager()->isLogin()) {
        //qDebug() << __FILE__ << __func__ << __LINE__ << config->id() << _toolbox->httpAPIManager()->getUserManager()->getToken();
        mavlink_message_t message;
        mavlink_msg_bind_flyer_plane_pack_chan(_mavlinkProtocol->getSystemId(),
                                               _mavlinkProtocol->getComponentId(),
                                               link->mavlinkChannel(),
                                               &message,
                                               QString(_toolbox->httpAPIManager()->getUserManager()->getToken()).toStdString().c_str(),
                                               config->id().toStdString().c_str());
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN] = {0};
        int len = mavlink_msg_to_send_buffer(buffer, &message);
        //qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << QByteArray((char*)buffer,len).toHex();
    ...
    _currentConnectedLink = nullptr;
#else
    if (!_toolbox->httpAPIManager()->getUserManager()->isLogin()) return;
    //qDebug() << __FILE__ << __func__ << __LINE__ << "binding...";
    for (int i=0; i<_tcpBindLinks.count(); i++) {
        LinkInterface* tcpLink = _tcpBindLinks[i].get();
        if (tcpLink && tcpLink->linkConfiguration()->type() == LinkConfiguration::TypeTcp) {
            TCPConfiguration* config = dynamic_cast<TCPConfiguration*>(tcpLink->linkConfiguration().get());
            if (config && config->id().compare("-1") != 0) {
                //qDebug() << __FILE__ << __func__ << __LINE__ << config->id() << _toolbox->httpAPIManager()->getUserManager()->getToken();
                mavlink_message_t message;
                mavlink_msg_bind_flyer_plane_pack_chan(_mavlinkProtocol->getSystemId(),
                                                       _mavlinkProtocol->getComponentId(),
                                                       tcpLink->mavlinkChannel(),
                                                       &message,
                                                       QString(_toolbox->httpAPIManager()->getUserManager()->getToken()).toStdString().c_str(),
                                                       config->id().toStdString().c_str());
                uint8_t buffer[MAVLINK_MAX_PACKET_LEN] = {0};
                int len = mavlink_msg_to_send_buffer(buffer, &message);
                //qDebug() << __FILE__ << __LINE__ << __func__ << __LINE__ << QByteArray((char*)buffer,len).toHex();
                tcpLink->writeBytesThreadSafe((const char*)buffer, len);
            }
        }
    }
...
void LinkManager::removeBindLink(LinkInterface *link)
{
    if (link == nullptr || !containsLink(link)) return;
    for (int i = 0; i < _tcpBindLinks.count(); i++) {
        if (_tcpBindLinks[i].get() == link) {
            _tcpBindLinks.removeAt(i);
            break;
