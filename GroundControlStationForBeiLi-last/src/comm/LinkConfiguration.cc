    ...
    , _isLinked     (false)
{
    ...
    connect(this, &LinkConfiguration::linkChanged, this, &LinkConfiguration::onLinkChanged);
}
...
void LinkConfiguration::onLinkChanged()
{
    LinkInterface *link = this->link();
    if (link) {
        connect(link, &LinkInterface::connected, this, &LinkConfiguration::onConnected);
        connect(link, &LinkInterface::disconnected, this, &LinkConfiguration::onDisconnected);
    } else {
        onDisconnected();
    }
}

void LinkConfiguration::onConnected()
{
    setLinked(true);
}

void LinkConfiguration::onDisconnected()
{
    setLinked(false);
}
...
void LinkConfiguration::setLinked(bool isLinked)
{
    if (_isLinked != isLinked) {
        _isLinked = isLinked;
        emit linkedChanged();
    }
}
