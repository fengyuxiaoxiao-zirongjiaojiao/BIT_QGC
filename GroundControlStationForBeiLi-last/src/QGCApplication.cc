...
void QGCApplication::_getVersionFromFile()
{
    QFile file("version.txt");
    if (file.open(QFileDevice::ReadOnly)) {
        QString versionText = "";
        versionText = file.readLine();
        file.close();
        int majorVersion, minorVersion, buildVersion;
        if (_parseVersionText(versionText, majorVersion, minorVersion, buildVersion)) {
            _majorVersion = majorVersion;
            _minorVersion = minorVersion;
            _buildVersion = buildVersion;
            this->setApplicationVersion(QString("%1.%2.%3").arg(majorVersion).arg(minorVersion).arg(buildVersion));
        }
    }
}
...
bool QGCApplication::_initForNormalAppBoot()
{
    ...
    toolbox()->corePlugin()->createStartWindow(_qmlAppEngine);

    connect(toolbox()->settingsManager()->appSettings(), &AppSettings::enterAppChanged, [=](){
        if (toolbox()->settingsManager()->appSettings()->enterApp()) {
            QSettings settings;
            if (_qmlAppEngine && _qmlAppEngine->rootObjects().size() > 0) {
                QObject *obj = _qmlAppEngine->rootObjects()[0];
                if (obj) obj->deleteLater();
                _qmlAppEngine->rootObjects().removeAt(0);
            }
...
QObject* QGCApplication::_rootQmlObject()
{
    if (_qmlAppEngine && _qmlAppEngine->rootObjects().size())
        return _qmlAppEngine->rootObjects().last();//return _qmlAppEngine->rootObjects()[0];
    return nullptr;
}
