...
#endif
    _httpAPIManager         = toolbox->httpAPIManager();
    _kmlManager             = toolbox->kmlManager();
    _controlCenterManager   = toolbox->controlCenterManager();
    _webMsgManager          = toolbox->webMsgManager();
    _rvizManager            = toolbox->rvizManager();
}
...
bool QGroundControlQmlGlobal::defFlytouav()
{
#ifdef Flytouav
    return true;
#else
