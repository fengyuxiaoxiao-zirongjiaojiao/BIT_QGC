...
void SurveyComplexItem::setSurveyPolygonFormKml()
{
    if (!qgcApp()->toolbox()->kmlManager()->polygon()->coordinateList().isEmpty()) {
        _surveyAreaPolygon.appendVertices(qgcApp()->toolbox()->kmlManager()->polygon()->coordinateList());
        _kmlArea = qgcApp()->toolbox()->kmlManager()->polygon()->area();
    } else {
        QList<QGeoCoordinate> coordinateList = qgcApp()->toolbox()->kmlManager()->defaultPolygonVertices();
        _surveyAreaPolygon.appendVertices(coordinateList);
    }
}
...
{
    double t = 0;
    if (speedSection() && speedSection()->available() && speedSection()->specifyFlightSpeed()) {
        double speed = speedSection()->flightSpeed()->rawValue().toDouble();
        t = speed == 0 ? 0 : triggerDistance() / speed;
    } else {
        t = _vehicleSpeed == 0 ? 0 : triggerDistance() / _vehicleSpeed;
    }
    return t;
}
