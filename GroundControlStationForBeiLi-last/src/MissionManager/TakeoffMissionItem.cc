...
void TakeoffMissionItem::setCoordinate(const QGeoCoordinate& coordinate)
{
    if (coordinate != this->coordinate()) {
        ...
        if (isFirst()) {
            ...
            if (preSimpleMissionItem()) {
                preSimpleMissionItem()->setCoordinate(coordinate);
            }
        }
    }
}
...
bool TakeoffMissionItem::isFirst()
{
    return false;
}

VisualMissionItem* TakeoffMissionItem::preSimpleMissionItem()
{
    int seq = this->sequenceNumber();
    for (int i = 0; i < _masterController->missionController()->visualItems()->count(); i++) {
        VisualMissionItem* vitem = _masterController->missionController()->visualItems()->value<VisualMissionItem*>(i);
        if (vitem && vitem->sequenceNumber() == seq) {
            int viIndex = i - 1;
            for (int j = viIndex; j >= 0; j--) {
...
void TakeoffMissionItem::setLaunchCoordinate(const QGeoCoordinate& launchCoordinate)
{
    ...
    if (isFirst()) {
        ...
        if (preSimpleMissionItem() && launchCoordinate != preSimpleMissionItem()->coordinate()) {
            preSimpleMissionItem()->setCoordinate(launchCoordinate);
            emit launchCoordinateChanged(launchCoordinate);
        }
    }
}
