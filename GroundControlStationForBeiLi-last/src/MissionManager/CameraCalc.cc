...
const char* CameraCalc::valueSetIsFrontalOverlapName =      "ValueSetIsFrontalOverlap";
const char* CameraCalc::valueSetIsSideOverlapName =         "ValueSetIsSideOverlap";
...
CameraCalc::CameraCalc(PlanMasterController* masterController, const QString& settingsGroup, QObject* parent)
    ...
    , _knownCameraList              (qgcApp()->toolbox()->settingsManager()->cameraInfoSettings()->cameraInfoList())
    ...
    , _valueSetIsFrontalOverlapFact (settingsGroup, _metaDataMap[valueSetIsFrontalOverlapName])
    , _valueSetIsSideOverlapFact    (settingsGroup, _metaDataMap[valueSetIsSideOverlapName])
{
    ...
    connect(&_valueSetIsFrontalOverlapFact, &Fact::valueChanged,                            this, &CameraCalc::_setDirty);
    connect(&_valueSetIsSideOverlapFact,    &Fact::valueChanged,                            this, &CameraCalc::_setDirty);
    ...
    connect(&_adjustedFootprintSideFact,    &Fact::rawValueChanged, this, &CameraCalc::_recalcTriggerDistance);
    connect(&_adjustedFootprintFrontalFact, &Fact::rawValueChanged, this, &CameraCalc::_recalcTriggerDistance);
    ...
    connect(qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings(), &VehicleInfoSettings::curCameraInfoChanged, this, &CameraCalc::_cameraInfoChanged);
    connect(qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings(), &VehicleInfoSettings::curCameraLandscapeChanged, [=](bool landscape){
        this->landscape()->setRawValue(QVariant(landscape));
    });

    if (qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings()->curCameraInfo()) {
        QString cameraName = qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings()->curCameraInfo()->canonicalName;
        if (cameraName.compare(_cameraNameFact.rawValueString()) != 0) {
    ...
    for (int cameraIndex=0; cameraIndex<_knownCameraList->count(); cameraIndex++) {
        ...
        CameraMetaData* cameraMetaData = _knownCameraList->value<CameraMetaData*>(cameraIndex);
...
void CameraCalc::_cameraInfoChanged()
{
    if (qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings()->curCameraInfo()) {
        QString cameraName = qgcApp()->toolbox()->settingsManager()->vehicleInfoSettings()->curCameraInfo()->canonicalName;
        _cameraNameFact.setRawValue(cameraName);
...
void CameraCalc::_recalcTriggerDistance(void)
{
    ...
    if (_valueSetIsSideOverlapFact.rawValue().toBool()) {
        _adjustedFootprintSideFact.setRawValue      (_imageFootprintSide * ((100.0 - _sideOverlapFact.rawValue().toDouble()) / 100.0));
    } else {
        _sideOverlapFact.setRawValue(100.0 - (_adjustedFootprintSideFact.rawValue().toDouble() / _imageFootprintSide) * 100.0);
    }

    if (_valueSetIsFrontalOverlapFact.rawValue().toBool()) {
        _adjustedFootprintFrontalFact.setRawValue   (_imageFootprintFrontal * ((100.0 - _frontalOverlapFact.rawValue().toDouble()) / 100.0));
    } else {
        _frontalOverlapFact.setRawValue(100.0 - (_adjustedFootprintFrontalFact.rawValue().toDouble() / _imageFootprintFrontal) * 100.0);
    }
    ...
    if (cameraBrand != _cameraBrand) {
        ...
        if (_cameraBrand != xlatManualCameraName() && _cameraBrand != xlatCustomCameraName()) {
            ...
            for (int cameraIndex=0; cameraIndex<_knownCameraList->count(); cameraIndex++) {
                ...
                firstCameraMetaData = _knownCameraList->value<CameraMetaData*>(cameraIndex);
