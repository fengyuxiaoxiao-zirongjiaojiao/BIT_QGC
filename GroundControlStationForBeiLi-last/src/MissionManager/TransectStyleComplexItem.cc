...
const char* TransectStyleComplexItem::altOffsetName =                       "AltOffset";
...
TransectStyleComplexItem::TransectStyleComplexItem(PlanMasterController* masterController, bool flyView, QString settingsGroup, QObject* parent)
    ...
    , _altOffsetFact                    (settingsGroup, _metaDataMap[altOffsetName])
    , _speedSection                     (nullptr)
{
    ...
    connect(&_altOffsetFact,                           &Fact::valueChanged,                 [=]{
        double distanceToSurface = _distanceToSurfaceNoAltOffset;
        double altoffset = _altOffsetFact.rawValue().toDouble();
        _cameraCalc.distanceToSurface()->setRawValue(distanceToSurface + altoffset);
    });
    connect(_cameraCalc.distanceToSurface(),           &Fact::valueChanged,                [=]{
        _distanceToSurfaceNoAltOffset = _cameraCalc.distanceToSurface()->rawValue().toDouble() - _altOffsetFact.rawValue().toDouble();
    });
...
void TransectStyleComplexItem::_save(QJsonObject& complexObject)
{
    ...
    innerObject[altOffsetName] = _altOffsetFact.rawValue().toDouble();
...
bool TransectStyleComplexItem::_load(const QJsonObject& complexObject, bool forPresets, QString& errorString)
{
    ...
    QList<JsonHelper::KeyValidateInfo> innerKeyInfoList = {
        ...
        { altOffsetName,                    QJsonValue::Double, false },        // 兼容没有航高偏置的plan文件
    };
    ...
    if (!forPresets) {
        ...
        for (const QJsonValue missionItemJson: missionItemsJsonArray) {
            ...
            bool isSpeedSection = false;
            if (speedSection()->available()) {
                if (missionItem->command() == MAV_CMD_DO_CHANGE_SPEED && missionItem->param3() == -1 && missionItem->param4() == 0 && missionItem->param5() == 0 && missionItem->param6() == 0 && missionItem->param7() == 0) {
                    if (_masterController->controllerVehicle()->multiRotor() && missionItem->param1() != 1) {
                        isSpeedSection = false;
                        continue;
                    } else if (_masterController->controllerVehicle()->fixedWing() && missionItem->param1() != 0) {
                        isSpeedSection = false;
                        continue;
                    }
                    speedSection()->flightSpeed()->setRawValue(missionItem->param2());
    ...
    _altOffsetFact.setRawValue                  (innerObject[altOffsetName].toDouble());
    _distanceToSurfaceNoAltOffset = _cameraCalc.distanceToSurface()->rawValue().toDouble() - _altOffsetFact.rawValue().toDouble();
...
int TransectStyleComplexItem::lastSequenceNumber(void) const
{
    if (_loadedMissionItems.count()) {
        ...
        return _sequenceNumber + itemCount - 1 + (_speedSection ? _speedSection->itemCount() : 0);
    } else {
...
void TransectStyleComplexItem::_appendSpeedSectionItems(QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum)
{
    if (!_addedSpeedSection) {
        _speedSection->appendSectionItems(items, missionItemParent, seqNum);
        _addedSpeedSection = true;
    }
}
...
void TransectStyleComplexItem::_buildAndAppendMissionItems(QList<MissionItem*>& items, QObject* missionItemParent)
{
    ...
    for (int coordIndex=0; coordIndex<_rgFlightPathCoordInfo.count(); coordIndex++) {
        ...
        case CoordTypeInteriorTerrainAdded:
            ...
            _appendSpeedSectionItems(items, missionItemParent, seqNum);
            break;
        case CoordTypeTurnaround:
        {
            bool firstEntryTurnaround   = coordIndex == 0;
...
void TransectStyleComplexItem::_updateOptionalSections()
{
    if (_speedSection) {
    ...
    connect(_speedSection,  &SpeedSection::dirtyChanged,                this, &TransectStyleComplexItem::_sectionDirtyChanged);
    connect(_speedSection,  &SpeedSection::itemCountChanged,            this, &TransectStyleComplexItem::_updateLastSequenceNumber);
    connect(_speedSection,  &SpeedSection::specifiedFlightSpeedChanged, this, &TransectStyleComplexItem::specifiedFlightSpeedChanged);

    emit speedSectionChanged(_speedSection);
    emit lastSequenceNumberChanged(lastSequenceNumber());
}

void TransectStyleComplexItem::_sectionDirtyChanged(bool dirty)
{
    if (dirty) {
        setDirty(true);
    }
...
void TransectStyleComplexItem::_updateLastSequenceNumber()
{
    emit lastSequenceNumberChanged(lastSequenceNumber());
}
