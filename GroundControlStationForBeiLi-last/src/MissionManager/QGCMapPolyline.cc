...
void QGCMapPolyline::_init(void)
{
    ...
    connect(this, &QGCMapPolyline::pathChanged, this, &QGCMapPolyline::_updateCenter);
}
...
QDomElement QGCMapPolyline::kmlPolylineElement(KMLDomDocument &domDocument)
{
#if 0
    <Polygon id="ID">
      <!-- specific to Polygon -->
...
//    domDocument.addTextElement(linearRingElement, "coordinates", coordString);
    ...
    QDomElement lineStringElement = domDocument.createElement("LineString");
    domDocument.appendChild(lineStringElement);

    domDocument.addTextElement(lineStringElement, "extruder",      "1");
    domDocument.addTextElement(lineStringElement, "tessellate",    "1");
    domDocument.addTextElement(lineStringElement, "altitudeMode",  "clampToGround");

    QString coordString;
    for (const QVariant& varCoord : _polylinePath) {
        coordString += QStringLiteral("%1\n").arg(domDocument.kmlCoordString(varCoord.value<QGeoCoordinate>()));
    }
    domDocument.addTextElement(lineStringElement, "coordinates", coordString);
...
void QGCMapPolyline::saveKMLToFile(const QString &kmlFile)
{
    if (kmlFile.isEmpty()) {
        return;
    }

    QString kmlFilename = kmlFile;
    if (!QFileInfo(kmlFile).fileName().contains(".")) {
        kmlFilename += QString(".%1").arg(AppSettings::kmlFileExtension);
    }

    QFile file(kmlFilename);

    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qgcApp()->showAppMessage(tr("KML save error %1 : %2").arg(kmlFile).arg(file.errorString()));
    } else {
        KMLSaveDomDocument saveKML;
        QDomElement placemarkElement = saveKML.addPlacemark(QStringLiteral("Polyline Area"), true);
        QDomElement polygonElement = kmlPolylineElement(saveKML);

        placemarkElement.appendChild(polygonElement);
        saveKML.addTextElement(placemarkElement, "styleUrl", QStringLiteral("#%1").arg(saveKML.polylineStyleName));
        saveKML.appendChildToRoot(placemarkElement);
        QTextStream stream(&file);
        stream << saveKML.toString();
        file.close();
    }
}
...
void QGCMapPolyline::_updateCenter()
{
    QGeoCoordinate center;

    if (_polylinePath.count() > 2) {
        QPointF centroid(0, 0);
        for (int i = 0; i < _polylinePath.count(); i++) {
            centroid += _pointFFromCoord(_polylinePath[i].value<QGeoCoordinate>());
        }
        center = _coordFromPointF(QPointF(centroid.x() / _polylinePath.count(), centroid.y() / _polylinePath.count()));
    }
    if (_center != center) {
        _center = center;
    }
