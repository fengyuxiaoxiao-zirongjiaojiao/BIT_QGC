...
VisualMissionItem* MissionController::insertTakeoffItem(QGeoCoordinate /*coordinate*/, int visualItemIndex, bool makeCurrentItem)
{
    ...
    {
        for (int j = visualItemIndex - 1; j >= 0; j--) {
            VisualMissionItem* preItem = _masterController->missionController()->visualItems()->value<VisualMissionItem*>(j);
            if (preItem && preItem->specifiesCoordinate() && !preItem->isTakeoffItem()) {
                _takeoffMissionItem->setCoordinate(preItem->coordinate());
                break;
            }
        }
    }
...
VisualMissionItem* MissionController::insertComplexMissionItem(QString itemName, QGeoCoordinate mapCenterCoordinate, int visualItemIndex, bool makeCurrentItem)
{
    ...
    if (itemName == SurveyComplexItem::name) {
        ...
        FixedWingLandingComplexItem* fixedWing = new FixedWingLandingComplexItem(_masterController, _flyView, _visualItems /* parent */);
        fixedWing->setLandingCoordinate(plannedHomePosition());
        newItem = fixedWing;
    } else if (itemName == VTOLLandingComplexItem::name) {
        VTOLLandingComplexItem* vtol = new VTOLLandingComplexItem(_masterController, _flyView, _visualItems /* parent */);
        vtol->setLandingCoordinate(plannedHomePosition());
        newItem = vtol;
            ...
            if (item->load(stream)) {
                if (firstItem && plannedHomePositionInFile) {
                    ...
                    if (TakeoffMissionItem::isTakeoffCommand(static_cast<MAV_CMD>(item->command()))) {
                        ...
                        TakeoffMissionItem *takeoffItem = new TakeoffMissionItem(item->missionItem(), _masterController, _flyView, settingsItem, visualItems);
                        item->deleteLater();
                        item = takeoffItem;
                    }
                    visualItems->append(item);
...
FlightPathSegment* MissionController::_createFlightPathSegmentWorker(VisualItemPair& pair)
{
    ...
    connect(pair.first,  &VisualMissionItem::exitCoordinateChanged, this,    &MissionController::_recalcDottedFlightPathSegments);
    connect(pair.second, &VisualMissionItem::coordinateChanged,     this,    &MissionController::_recalcDottedFlightPathSegments);
...
void MissionController::_recalcFlightPathSegments(void)
{
    ...
    bool isCar = false;
    if (_settingsItem && _settingsItem->carSection()->specifyCarAndPlane()) {
        isCar = _settingsItem->carSection()->carAndPlane()->rawValue().toBool();
    }
    for (int i=1; i<_visualItems->count(); i++) {
        VisualMissionItem*  visualItem =    qobject_cast<VisualMissionItem*>(_visualItems->get(i));
        SimpleMissionItem*  simpleItem =    qobject_cast<SimpleMissionItem*>(visualItem);
        ...
        if (visualItem->specifiesCoordinate() && !visualItem->isStandaloneCoordinate()) {
            ...
            if (complexItem && complexItem->isIncomplete()) {
                ...
                if (lastFlyThroughVI != _settingsItem || (homePositionValid && linkStartToHome)) {
                    ...
                    if (true/*!_flyView || addDirectionArrow*/) {
        ...
        if (simpleItem) {
            if (simpleItem->carSection()->specifyCarAndPlane()) {
                isCar = simpleItem->carSection()->carAndPlane()->rawValue().toBool();
            }
        }
    }
    ...
    if (lastSegmentVisualItemPair.first) {
        ...
        if (_flightPathSegmentHashTable.contains(lastSegmentVisualItemPair)) {
            ...
            coordVector = new FlightPathSegment(
                        ...
                        this);
            coordVector->setDottedLine(isCar);
    ...
    _recalcDottedFlightPathSegments();
}

void MissionController::_recalcDottedFlightPathSegments()
{
    if (_distanceMapPoint <= 0) _distanceMapPoint = 20;
    _dottedFlightPathSegments.beginReset();
    _dottedFlightPathSegments.clear();
    for (int i = 0; i < _simpleFlightPathSegments.count(); i++) {
        FlightPathSegment* segment = _simpleFlightPathSegments.value<FlightPathSegment*>(i);
        QGeoCoordinate coord1 = segment->coordinate1();
        QGeoCoordinate coord2 = segment->coordinate2();
        int distance = coord1.distanceTo(coord2);
        if (distance == 0) continue;
        double coord1AMSLAlt = segment->coord1AMSLAlt();
        double coord2AMSLAlt = segment->coord2AMSLAlt();
        double slope =      (coord2AMSLAlt - coord1AMSLAlt) / distance;
        double azimuth = coord1.azimuthTo(coord2);

        if (!segment->dottedLine()) {
            FlightPathSegment* segment_line = new FlightPathSegment(segment->segmentType(), coord1, coord1AMSLAlt, coord2, coord2AMSLAlt, !_flyView, this);
            _dottedFlightPathSegments.append(segment_line);
            continue;
        }

        int preDis = distance + 10000;
        QGeoCoordinate first = coord1;
        while (distance > _distanceMapPoint) {
            if (distance >= preDis) {
                qDebug() << __FILE__ << __func__ << __LINE__  << "error" << preDis << distance;
                break;
            }
            QGeoCoordinate second = first.atDistanceAndAzimuth(_distanceMapPoint, azimuth);
            coord2AMSLAlt = slope * _distanceMapPoint + coord1AMSLAlt;
            if (distance <= _distanceMapPoint*2) {
                second = segment->coordinate2();
                coord2AMSLAlt = segment->coord2AMSLAlt();
            }
            FlightPathSegment* segment_n = new FlightPathSegment(segment->segmentType(), first, coord1AMSLAlt, second, coord2AMSLAlt, !_flyView, this);

            _dottedFlightPathSegments.append(segment_n);

            first = second.atDistanceAndAzimuth(_distanceMapPoint, azimuth);
            preDis = distance;
            //distance = distance - _distanceMapPoint*2;
            distance = second.distanceTo(segment->coordinate2());

            coord1AMSLAlt = slope * _distanceMapPoint*2 + coord1AMSLAlt;
        }
    }
    _dottedFlightPathSegments.endReset();
}
...
int MissionController::takeoffCommand(void)
{
    return _controllerVehicle->vtol() ? MAV_CMD_NAV_VTOL_TAKEOFF : MAV_CMD_NAV_TAKEOFF;
}

int MissionController::landCommand(void)
{
    return _controllerVehicle->vtol() ? MAV_CMD_NAV_VTOL_LAND : MAV_CMD_NAV_LAND;
}

void MissionController::distanceMapPoint(const QGeoCoordinate coord1, const QGeoCoordinate coord2)
{
    double distanceMapPoint = coord1.distanceTo(coord2);
    if (distanceMapPoint <= 1) {
        distanceMapPoint = 20;
    }
    if (distanceMapPoint != _distanceMapPoint) {
        _distanceMapPoint = distanceMapPoint;
        _recalcDottedFlightPathSegments();
    }
}
...
QList<MissionItem *> MissionController::missionItems()
{
    QList<MissionItem*> rgMissionItems;
    _convertToMissionItems(_visualItems, rgMissionItems, this);
    return rgMissionItems;
}
