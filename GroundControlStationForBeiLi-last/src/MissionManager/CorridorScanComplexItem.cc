...
void CorridorScanComplexItem::setCorridorPolylineFormKml()
{
    if (!qgcApp()->toolbox()->kmlManager()->polyline()->coordinateList().isEmpty()) {
        _corridorPolyline.appendVertices(qgcApp()->toolbox()->kmlManager()->polyline()->coordinateList());
        _kmlLength = qgcApp()->toolbox()->kmlManager()->polyline()->length();
    } else if (!qgcApp()->toolbox()->kmlManager()->polygon()->coordinateList().isEmpty()) {
        QList<QGeoCoordinate> coordinateList = qgcApp()->toolbox()->kmlManager()->polygon()->coordinateList();
        coordinateList.append(coordinateList.first());
        _corridorPolyline.appendVertices(coordinateList);
        _kmlLength = qgcApp()->toolbox()->kmlManager()->polygon()->length();
    } else {
        QList<QGeoCoordinate> coordinateList = qgcApp()->toolbox()->kmlManager()->defaultPolylineVertices();
        _corridorPolyline.appendVertices(coordinateList);
    }
}
