...
bool MissionItem::specifiedCarAndPlane() const
{
    bool isCar = std::numeric_limits<bool>::quiet_NaN();

    if (_commandFact.rawValue().toInt() == MAV_CMD_DO_SET_SERVO && _param1Fact.rawValue().toInt() == 1) {
        isCar = _param2Fact.rawValue().toInt() == 1300;
    }
...
void MissionItem::_param2Changed(QVariant value)
{
    ...
    bool isCar = specifiedCarAndPlane();
    if (_commandFact.rawValue().toInt() == MAV_CMD_DO_SET_SERVO && _param1Fact.rawValue().toInt() == 1) {
        emit specifiedCarAndPlaneChanged(isCar);
    }
}
