...
public:
    ...
    QGeoCoordinate  launchCoordinate            (void)  {
        if (isFirst()) return _settingsItem->coordinate();
        if (isFirst()) return _settingsItem->coordinate();
        else {
            if (preSimpleMissionItem()) {
                return preSimpleMissionItem()->coordinate();
            }
        }
    ...
    VisualMissionItem* preSimpleMissionItem();
signals:
