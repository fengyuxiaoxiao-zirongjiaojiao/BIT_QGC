...
void QGCMapPolygon::saveKMLToFile(const QString &kmlFile)
{
    if (kmlFile.isEmpty()) {
        return;
    }
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QDomElement placemarkElement = saveKML.addPlacemark(QStringLiteral("Polygon Area"), true);
        QDomElement polygonElement = kmlPolygonElement(saveKML);

        placemarkElement.appendChild(polygonElement);
        saveKML.addTextElement(placemarkElement, "styleUrl", QStringLiteral("#%1").arg(saveKML.polygonStyleName));
...
double QGCMapPolygon::length(void) const
{
    double length = 0;

    for (int i=0; i<_polygonPath.count() - 1; i++) {
        QGeoCoordinate from = _polygonPath[i].value<QGeoCoordinate>();
        QGeoCoordinate to = _polygonPath[i+1].value<QGeoCoordinate>();
        length += from.distanceTo(to);
    }
    if (!_polygonPath.isEmpty()) {
        QGeoCoordinate from = _polygonPath.last().value<QGeoCoordinate>();
        QGeoCoordinate to = _polygonPath.first().value<QGeoCoordinate>();
        length += from.distanceTo(to);
    }
    return length;
}
