...
bool SimpleMissionItem::specifiedCarAndPlane()
{
    if (_carSection->specifyCarAndPlane()) {
        return _carSection->carAndPlane()->rawValue().toBool();
    } else {
        return missionItem().specifiedCarAndPlane();
    }
}
...
bool SimpleMissionItem::scanForSections(QmlObjectListModel* visualItems, int scanIndex, PlanMasterController* /*masterController*/)
{
    ...
    if (_carSection->available()) {
        sectionFound |= _carSection->scanForSection(visualItems, scanIndex);
    }
...
void SimpleMissionItem::_updateOptionalSections(void)
{
    ...
    _carSection = new CarSection(_masterController, this);
    if (static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_WAYPOINT || static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_TAKEOFF
             || static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_RETURN_TO_LAUNCH
            || static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_LAND
            || static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_VTOL_TAKEOFF
            || static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_VTOL_LAND) {
        _cameraSection->setAvailable(true);
        _speedSection->setAvailable(true);
        _carSection->setAvailable(true);
    }
    ...
    connect(_carSection,  &CarSection::dirtyChanged,                this, &SimpleMissionItem::_sectionDirtyChanged);
    connect(_carSection,  &CarSection::itemCountChanged,            this, &SimpleMissionItem::_updateLastSequenceNumber);
    connect(_carSection,  &CarSection::specifiedCarAndPlaneChanged, this, &SimpleMissionItem::specifiedCarAndPlaneChanged);

    emit cameraSectionChanged(_cameraSection);
    emit speedSectionChanged(_speedSection);
    emit carSectionChanged(_carSection);
...
void SimpleMissionItem::appendMissionItems(QList<MissionItem*>& items, QObject* missionItemParent)
{
    ...
    _carSection->appendSectionItems(items, missionItemParent, seqNum);
}
