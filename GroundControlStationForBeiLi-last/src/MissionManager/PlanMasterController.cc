...
PlanMasterController *PlanMasterController::_instance = nullptr;
...
{
    qCDebug(PlanMasterControllerLog) << "PlanMasterController::sendToVehicle Mission Item send complete";
    if (_sendGeoFence) {
        _sendGeoFence = false;
...
void PlanMasterController::sendToVehicles()
{
    QJsonDocument jsonDoc = saveToJson();
    for (int i = 0; i < _multiVehicleMgr->vehicles()->count(); i++) {
        Vehicle* vehicle = qobject_cast<Vehicle*>(_multiVehicleMgr->vehicles()->get(i));
        if (vehicle && _ids.contains(QString::number(vehicle->id()))) {
            PlanMasterController::sendPlanToVehicle(vehicle, jsonDoc);
        }
    }
    setDirty(false);
}
...
void PlanMasterController::loadFromJson(const QJsonDocument &jsonDoc)
{
    QString errorString;
    QString errorMessage = tr("Error loading json document (%1)");
...
void PlanMasterController::saveToFile(const QString& filename)
{
    ...
    if (!QFileInfo(filename).fileName().contains(".")) {
        ...
        if (QFileInfo(filename).fileName().split(".").last().compare(QString(AppSettings::waypointsFileExtension), Qt::CaseInsensitive) == 0) {
            saveToWaypoints(filename);
            return;
        }
    }
...
void PlanMasterController::saveToWaypoints(const QString &filename)
{
    if (filename.isEmpty()) {
          return;
      }

      QString waypointsFilename = filename;
      if (!QFileInfo(filename).fileName().contains(".")) {
          waypointsFilename += QString(".%1").arg(AppSettings::waypointsFileExtension);
      }

      QFile file(waypointsFilename);

      if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
          qgcApp()->showAppMessage(tr("Plan save error %1 : %2").arg(filename).arg(file.errorString()));
      } else {
          QList<MissionItem*> missionItems = missionController()->missionItems();
          QTextStream out( &file );

          out<<"QGC WPL 110"<<"\n";
          out << "0\t1\t0\t16\t0\t0\t0\t0\t" << QString::number(NAN_TO_0(missionItems.at(0)->param5()), 'f', 8) << "\t"
              << QString::number(NAN_TO_0(missionItems.at(0)->param6()), 'f', 8) << "\t"
              << QString::number(NAN_TO_0(missionItems.at(0)->param7()), 'f', 8) << "\t1\n";
          for (int i = 1; i < missionItems.size(); i++) {
              MissionItem* missionItem = missionItems.at(i);
              out << QString::number(missionItem->sequenceNumber()) << "\t" << QString::number(missionItem->isCurrentItem() ? 1 : 0)
                  << "\t" << QString::number(missionItem->frame()) << "\t" << QString::number(missionItem->command())
                  << "\t" << QString::number(NAN_TO_0(missionItem->param1()), 'f', 8) << "\t" << QString::number(NAN_TO_0(missionItem->param2()), 'f', 8)
                  << "\t" << QString::number(NAN_TO_0(missionItem->param3()), 'f', 8) << "\t" << QString::number(NAN_TO_0(missionItem->param4()), 'f', 8)
                  << "\t" << QString::number(NAN_TO_0(missionItem->param5()), 'f', 8) << "\t" << QString::number(NAN_TO_0(missionItem->param6()), 'f', 8)
                  << "\t" << QString::number(NAN_TO_0(missionItem->param7()), 'f', 8) << "\t1\n";
          }
          file.close();
      }
}
...
void PlanMasterController::setSelectedVehicleIds(const QStringList &ids)
{
    _ids = ids;
}
...
QStringList PlanMasterController::saveNameFilters(void) const
{
    ...
    filters << tr("Plan Files (*.%1)").arg(fileExtension()) << tr("Mission(*.%1)").arg(AppSettings::waypointsFileExtension) << tr("All Files (*)");
    return filters;
}
...
void PlanMasterController::sendPlanToVehicle(Vehicle *vehicle, const QJsonDocument &jsonDoc)
{
    PlanMasterController* controller = new PlanMasterController();
    controller->startStaticActiveVehicle(vehicle, true /* deleteWhenSendCompleted */);
    controller->loadFromJson(jsonDoc);
    controller->sendToVehicle();
}
