...
Item {
    ...
    property alias editorMap: editorMap
    ...
    property bool   _showWaypointList:                  false
    ...
    readonly property int       _layerKmlPolygon:           4
    readonly property int       _layerKmlPolyline:          5
    readonly property string    _armedVehicleUploadPrompt:  qsTr("Vehicle is currently armed. Do you want to upload the mission to the vehicle?")

    property var zoomLevel: editorMap.zoomLevel

    function setEditingLayer(layer) {
        _editingLayer = layer
    }
    function setAddWaypointRallyPointActionChecked(checked) {
        addWaypointRallyPointAction.checked = checked
    }
    ...
    function updateDottedSpace() {
        var x1 = editorMap.centerViewport.x
        var y1 = editorMap.centerViewport.y
        var coord1 = editorMap.toCoordinate(Qt.point(x1, y1), false)
        var coord2 = editorMap.toCoordinate(Qt.point(x1 + 20, y1), false)
    ...
    PlanMasterController {
        ...
        Component.onCompleted: {
            ...
            QGroundControl.webMsgManager.setPlanMasterController(_planMasterController)
        }

        onPromptForPlanUsageOnVehicleChange: {
            if (!_promptForPlanUsageShowing) {
                _promptForPlanUsageShowing = true
                // vincent_xjw 2022年5月24日 飞机切换/断开连接后提示是否保存航线的提示
    ...
    Item {
        ...
        FlightMap {
            ...
            // Add the mission item visuals to the map 航点
            Repeater {
...
//            }
            ...
            MapItemView {
                ...
                delegate: MapPolyline {
                    ...
                    line.color: _terrainCollision ? "red" : QGroundControl.globalPalette.mapMissionTrajectory
            ...
            // KML Area polygon
            QGCMapKMLPolygonVisuals {
                id:                 mapKmlPolygonVisuals
                mapControl:         editorMap
                mapPolygon:         QGroundControl.kmlManager.polygon
                interactive:        _editingLayer === _layerKmlPolygon
                borderWidth:        6
                borderColor:        "#800080"
                interiorColor:      "#ffd400"
                altColor:           QGroundControl.globalPalette.surveyPolygonTerrainCollision
                interiorOpacity:    0.5 * _root.opacity
                polygonTraceMode:   QGroundControl.kmlManager.polygonTraceMode
            }

            // KML Area polyline
            QGCMapKMLPolylineVisuals {
                id:             mapKmlPolylineVisuals
                mapControl:     editorMap
                mapPolyline:    QGroundControl.kmlManager.polyline
                interactive:    _editingLayer === _layerKmlPolyline
                lineWidth:      6
                lineColor:      "#800080"
                //visible:        _editingLayer === _layerKmlPolyline
                interiorOpacity:        0.5 * _root.opacity
                polylineTraceMode:   QGroundControl.kmlManager.polylineTraceMode
            }
...
// vincent start 航点编辑
        ...
        Rectangle {
            ...
            anchors.right: _showWaypointList ? rightPanel.left : parent.right

            Image {
                anchors.centerIn: parent
                source: "qrc:/qmlimages/BeiLi/list_btn.png"
            }
        ...
        Rectangle {
            id:                 rightPanel
            height:             parent.height - ((parent.width * 145) / 1920) - 64 - 20//parent.height
            ...
            visible: _showWaypointList // vincent_xjw 暂时不显示  航点编辑的背景框
        }
        //-------------------------------------------------------
        // Right Panel Controls
        Item {
            visible: _showWaypointList  // vincent_xjw 暂时不显示  航点编辑
...
// vincent_xjw  end 航点编辑
//        Item {
//            id: missionItemEditorRoot
//            width: _rightPanelWidth
//            anchors.right: parent.right
//            anchors.rightMargin: _toolsMargin
//            anchors.verticalCenter: parent.verticalCenter
...
//                property int itemsCount: _missionController.visualItems.count
...
//                        preOrNextTab.curIndex = preOrNextTab.searchIndex(_missionController.currentPlanViewSeqNum)
//                    }
//                }

//                QGCButton {
//                    text:       qsTr("上一个")
//                    enabled: (preOrNextTab.curIndex - 1 >= 0)
//                    onClicked: {
//                        preOrNextTab.curIndex = preOrNextTab.curIndex - 1
//                        var sequenceNumber = _missionController.visualItems.get(preOrNextTab.curIndex).sequenceNumber
//                        _missionController.setCurrentPlanViewSeqNum(sequenceNumber, false)
//                    }
//                }
//                QGCButton {
//                    text:       qsTr("下一个")
//                    enabled: (preOrNextTab.curIndex + 1 < _missionController.visualItems.count)
//                    onClicked: {
//                        preOrNextTab.curIndex = preOrNextTab.curIndex + 1
//                        var sequenceNumber = _missionController.visualItems.get(preOrNextTab.curIndex).sequenceNumber
//                        _missionController.setCurrentPlanViewSeqNum(sequenceNumber, false)
...
//                }
//                Text {
//                    text: String("%1/%2").arg((preOrNextTab.curIndex + 1)).arg(preOrNextTab.itemsCount)
...
//                model: _missionController.visualItems
//                MissionItemEditor {
//                    visible:        _missionController.currentPlanViewSeqNum === object.sequenceNumber
//                    //anchors.centerIn: missionItemEditorRoot
//                    anchors.top: preOrNextTab.bottom
//                    anchors.topMargin: _toolsMargin
//                    anchors.right: missionItemEditorRoot.right
//                    map:            editorMap
//                    masterController:  _planMasterController
//                    missionItem:    object
//                    width:          _rightPanelWidth
...
// vincent_xjw new 航点编辑
        ...
        TerrainStatus {
            ...
            visible:            false//vincent_xjw 地形状态暂时不显示_internalVisible && _editingLayer === _layerMission && QGroundControl.corePlugin.options.showMissionStatus
        ...
        MapScale {
            id:                     mapScale
            visible:                false//vincent_xjw 地图缩放工具暂时不显示
            anchors.margins:        _toolsMargin
