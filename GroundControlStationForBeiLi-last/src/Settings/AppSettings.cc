...
DECLARE_SETTINGSFACT(AppSettings, mapView3d)
DECLARE_SETTINGSFACT(AppSettings, pw)
...
QString AppSettings::appVersion()
{
    return qgcApp()->appVersion();
}

void AppSettings::setPassword(const QString &pw)
{
    QString md5;
    QString header = "BIT";
    header += pw;
    QByteArray bb = QCryptographicHash::hash(header.toUtf8(), QCryptographicHash::Md5);
    md5.append(bb.toHex());
    this->pw()->setRawValue(md5);
}

bool AppSettings::loginDeveloper(const QString &pw)
{
    QString md5;
    QString header = "BIT";
    header += pw;
    QByteArray bb = QCryptographicHash::hash(header.toUtf8(), QCryptographicHash::Md5);
    md5.append(bb.toHex());
    QString p = this->pw()->rawValueString();
    return (md5 == p);
}

bool AppSettings::changePassword(const QString &oldPw, const QString &newPw)
{
    if (loginDeveloper(oldPw)) {
        setPassword(newPw);
        return true;
    }
    return false;
