...
public:
    ...
    DEFINE_SETTINGFACT(mapView3d)

    DEFINE_SETTINGFACT(pw)
    ...
    Q_PROPERTY(bool enterApp        READ enterApp   WRITE setEnterApp   NOTIFY enterAppChanged)
    Q_PROPERTY(int bootAppMode      READ bootAppMode CONSTANT)
    ...
    bool enterApp() { return _enterApp; }
    void setEnterApp(bool enter) { if (enter != _enterApp) { _enterApp = enter; emit enterAppChanged(); } }
    Q_INVOKABLE void setBootAppMode(int mode) { _appMode = mode; }
    int bootAppMode(void) { return _appMode; }

    QString appVersion(void);

    Q_INVOKABLE void setPassword(const QString& pw);
    Q_INVOKABLE bool loginDeveloper(const QString& pw);
    Q_INVOKABLE bool changePassword(const QString& oldPw, const QString& newPw);
    Q_INVOKABLE bool invalidPassword() { return this->pw()->rawValue().toString().isEmpty(); }
