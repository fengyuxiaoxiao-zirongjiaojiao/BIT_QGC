...
void VehicleBatteryFactGroup::_handleSysStatus(Vehicle *vehicle, mavlink_message_t &message)
{
    mavlink_sys_status_t sysStatus;
    mavlink_msg_sys_status_decode(&message, &sysStatus);
    VehicleBatteryFactGroup* group = _findOrAddBatteryGroupById(vehicle, 0);
    if (group) {
        group->current()->setRawValue(sysStatus.current_battery == -1 ? qQNaN() : sysStatus.current_battery / 100.0f);
        group->voltage()->setRawValue(sysStatus.voltage_battery == INT16_MAX ? qQNaN() : sysStatus.voltage_battery / 1000.0f);
        group->instantPower()->setRawValue((sysStatus.current_battery == -1|| sysStatus.voltage_battery == INT16_MAX)
                                           ? qQNaN() : (float)(sysStatus.current_battery * sysStatus.voltage_battery)/(100000.0));
        group->percentRemaining()->setRawValue(sysStatus.battery_remaining);
    }
}
