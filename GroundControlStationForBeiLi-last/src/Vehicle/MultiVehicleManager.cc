...
#include "HttpServer/UAVInfoManager.h"
    ...
    , _gcsHeartbeatEnabled(true)
{
    ...
    _colors << "#68FF2C"
           << "#0072FF" << "#4C9DFF" << "#7FB9FF" << "#B2E2FF"
           << "#DA0000" << "#F24949" << "#F27979" << "#F2AAAA"
           << "#5002D1" << "#6A2AD1" << "#9169D1" << "#B7A7D1"
           << "#DECB00" << "#FFE674" << "#FFE600" << "#FFD52B";

    _uavIcons << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_highlight.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_01.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_02.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_03.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_04.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_05.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_06.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_07.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_08.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_09.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_10.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_11.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_12.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_13.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_14.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_15.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/uav_16.png";

    _carIcons << "qrc:/qmlimages/BeiLi/vehicleIcon/car_highlight.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_01.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_02.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_03.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_04.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_05.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_06.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_07.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_08.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_09.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_10.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_11.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_12.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_13.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_14.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_15.png"
             << "qrc:/qmlimages/BeiLi/vehicleIcon/car_16.png";
}
...
void MultiVehicleManager::shutdown()
{
    while (_firstVehicles.count() > 0) {
        _firstVehicles.removeAt(0);
    }
    while (_secondVehicles.count() > 0) {
        _secondVehicles.removeAt(0);
    }
    while (_thirdVehicles.count() > 0) {
        _thirdVehicles.removeAt(0);
    }
    while (_fourthVehicles.count() > 0) {
        _fourthVehicles.removeAt(0);
    }
}
...
void MultiVehicleManager::_vehicleHeartbeatInfo(LinkInterface* link, int vehicleId, int componentId, int vehicleFirmwareType, int vehicleType)
{
    ...
    _insertVehicleForGroup(vehicle);
...
void MultiVehicleManager::_deleteVehiclePhase1(Vehicle* vehicle)
{
    ...
    for (int i=0; i<_vehicles.count(); i++) {
        if (_vehicles[i] == vehicle) {
            ...
            _removeVehicleForGroup(vehicle);
            found = true;
            break;
        }
    }
    ...
    for (int i=0; i<sharedLinks.count(); i++) {
        ...
        if (link->isConnected() && linkConfiguration && !linkConfiguration->isHighLatency()) {
            ...
            mavlink_msg_heartbeat_pack_chan(_mavlinkProtocol->getSystemId(),
                                            ...
                                            MAV_AUTOPILOT_ARDUPILOTMEGA,//MAV_AUTOPILOT_INVALID,   // MAV_AUTOPILOT
...
void MultiVehicleManager::_insertVehicleForGroup(Vehicle *vehicle)
{
    Vehicle *curVehicle = vehicle;
    UAVInfoManager *uavInfoManager = _toolbox->httpAPIManager()->getUAVInfoManager();
    QmlObjectListModel* firstUAVs = uavInfoManager->firstUAVs();
    QmlObjectListModel* secondUAVs = uavInfoManager->secondUAVs();
    QmlObjectListModel* thirdUAVs = uavInfoManager->thirdUAVs();
    QmlObjectListModel* fourthUAVs = uavInfoManager->fourthUAVs();
    bool hasDept = false;
    bool flag = false;
    int index = 0;
    if (!hasDept) {
        if (uavInfoManager->contains(firstUAVs, curVehicle->id())) {
            _firstVehicles.append(curVehicle);
            index = _firstVehicles.count();
            hasDept = true;

            if (_firstVehicles.count() > 4) {
                for (int i = 0; i < _firstVehicles.count(); i++) {
                    Vehicle* v = qobject_cast<Vehicle*>(_firstVehicles[i]);
                    if (!uavInfoManager->contains(firstUAVs, v->id())) {
                        curVehicle = v;
                        flag = true;
                        _firstVehicles.removeAt(i);
                        break;
                    }
                }
            }
    ...
    if (!hasDept) {
        if (uavInfoManager->contains(secondUAVs, curVehicle->id())) {
            _secondVehicles.append(curVehicle);
            index = 4 + _secondVehicles.count();
            hasDept = true;

            if (_secondVehicles.count() > 4) {
                for (int i = 0; i < _secondVehicles.count(); i++) {
                    Vehicle* v = qobject_cast<Vehicle*>(_secondVehicles[i]);
                    if (!uavInfoManager->contains(secondUAVs, v->id())) {
                        curVehicle = v;
                        flag = true;
                        _secondVehicles.removeAt(i);
                        break;
                    }
                }
            }
    ...
    if (!hasDept) {
        if (uavInfoManager->contains(thirdUAVs, curVehicle->id())) {
            _thirdVehicles.append(curVehicle);
            index = 8 + _thirdVehicles.count();
            hasDept = true;

            if (_thirdVehicles.count() > 4) {
                for (int i = 0; i < _thirdVehicles.count(); i++) {
                    Vehicle* v = qobject_cast<Vehicle*>(_thirdVehicles[i]);
                    if (!uavInfoManager->contains(thirdUAVs, v->id())) {
                        curVehicle = v;
                        flag = true;
                        _thirdVehicles.removeAt(i);
                        break;
                    }
                }
            }
    ...
    if (!hasDept) {
        if (uavInfoManager->contains(fourthUAVs, curVehicle->id())) {
            _fourthVehicles.append(curVehicle);
            index = 12 + _fourthVehicles.count();
            hasDept = true;

            if (_fourthVehicles.count() > 4) {
                for (int i = 0; i < _fourthVehicles.count(); i++) {
                    Vehicle* v = qobject_cast<Vehicle*>(_fourthVehicles[i]);
                    if (!uavInfoManager->contains(fourthUAVs, v->id())) {
                        curVehicle = v;
                        flag = true;
                        _fourthVehicles.removeAt(i);
                        break;
                    }
                }
            }
    ...
    // 未分组的飞机
    if (!hasDept || flag) {
        if (firstUAVs->count() < 4 && _firstVehicles.count() < 4) {
            _firstVehicles.append(curVehicle);
            index = _firstVehicles.count();
        } else if (secondUAVs->count() < 4 && _secondVehicles.count() < 4) {
            _secondVehicles.append(curVehicle);
            index = 4 + _secondVehicles.count();
        } else if (thirdUAVs->count() < 4 && _thirdVehicles.count() < 4) {
            _thirdVehicles.append(curVehicle);
            index = 8 + _thirdVehicles.count();
        } else if (fourthUAVs->count() < 4 && _fourthVehicles.count() < 4) {
            _fourthVehicles.append(curVehicle);
            index = 12 + _fourthVehicles.count();
        }
    }

    vehicle->setActiveColor(_colors.at(0));
    vehicle->setColor(_colors.at(index));
    vehicle->setActiveCarImage(_carIcons.at(0));
    vehicle->setActivePlaneImage(_uavIcons.at(0));
    vehicle->setCarImage(_uavIcons.at(index));
    vehicle->setPlaneImage(_uavIcons.at(index));
    vehicle->updateVehicleImageOpaque();
}

void MultiVehicleManager::_removeVehicleForGroup(Vehicle *vehicle)
{
    for (int i = 0; i < _firstVehicles.count(); i++) {
        if (_firstVehicles[i] == vehicle) {
            _firstVehicles.removeAt(i);
            break;
        }
    }
    for (int i = 0; i < _secondVehicles.count(); i++) {
        if (_secondVehicles[i] == vehicle) {
            _secondVehicles.removeAt(i);
            break;
        }
    }
    for (int i = 0; i < _thirdVehicles.count(); i++) {
        if (_thirdVehicles[i] == vehicle) {
            _thirdVehicles.removeAt(i);
            break;
        }
    }
    for (int i = 0; i < _fourthVehicles.count(); i++) {
        if (_fourthVehicles[i] == vehicle) {
            _fourthVehicles.removeAt(i);
            break;
        }
    }
