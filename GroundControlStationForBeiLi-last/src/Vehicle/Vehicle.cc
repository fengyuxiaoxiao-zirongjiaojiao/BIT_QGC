...
const char* Vehicle::_distanceToNextFactName =      "distanceToNext";
const char* Vehicle::_headingToHomeFactName =       "headingToHome";
const char* Vehicle::_distanceToGCSFactName =       "distanceToGCS";
const char* Vehicle::_hobbsFactName =               "hobbs";
const char* Vehicle::_throttlePctFactName =         "throttlePct";
const char* Vehicle::_soundHeadingFactName =        "soundHeading";
...
Vehicle::Vehicle(LinkInterface*             link,
    ...
    , _distanceToNextFact           (0, _distanceToNextFactName,    FactMetaData::valueTypeDouble)
    , _headingToHomeFact            (0, _headingToHomeFactName,     FactMetaData::valueTypeDouble)
    , _distanceToGCSFact            (0, _distanceToGCSFactName,     FactMetaData::valueTypeDouble)
    , _hobbsFact                    (0, _hobbsFactName,             FactMetaData::valueTypeString)
    , _throttlePctFact              (0, _throttlePctFactName,       FactMetaData::valueTypeUint16)
    , _soundHeadingFact             (0, _soundHeadingFactName,      FactMetaData::valueTypeInt32)
    , _gpsFactGroup                 (this)
    , _gps2FactGroup                (this)
    , _windFactGroup                (this)
    ...
    , _vehicleImageOpaque           ("")
    , _vehiclePathColor             ("#be781c")
{
    _linkManager = _toolbox->linkManager();
    _linkManager->removeBindLink(link);
...
void Vehicle::_commonInit()
{
    ...
    connect(this, &Vehicle::coordinateChanged,      this, &Vehicle::_updateHeadingToNextWP);
    ...
    _addFact(&_distanceToNextFact,      _distanceToNextFactName);
    _addFact(&_headingToHomeFact,       _headingToHomeFactName);
    _addFact(&_distanceToGCSFact,       _distanceToGCSFactName);
    _addFact(&_throttlePctFact,         _throttlePctFactName);
    _addFact(&_soundHeadingFact,        _soundHeadingFactName);
...
#endif
    _updateVehicleImageOpaque();
    connect(this, &Vehicle::isCarChanged, this, &Vehicle::_updateVehicleImageOpaque);
    connect(_toolbox->multiVehicleManager(), &MultiVehicleManager::activeVehicleChanged, this, &Vehicle::_updateVehicleImageOpaque);
}
...
void Vehicle::setCar(bool car)
{
    int value = car ? 1300 : 1700;
    sendMavCommand(_defaultComponentId, MAV_CMD_DO_SET_SERVO, true, 1, value, 0, 0, 0, 0);
}
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    /*case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW:
    {
        ...
        bool iscar = servoOut.servo1_raw <= 1300;
        if (iscar != _isCar) {
            _isCar = iscar;
            emit isCarChanged();
        }
...
void Vehicle::_handleExtendedSysState(mavlink_message_t& message)
{
    ...
    bool isCar = extendedState.vtol_state == 4; // 3:为飞机； 4：为小车
    if (isCar != _isCar) {
        _isCar = isCar;
        emit isCarChanged();
    }
...
QStringList Vehicle::flightModesForSimple()
{
    return _firmwarePlugin->flightModes(this, true);
}

QStringList Vehicle::flightModesZh()
{
    return _firmwarePlugin->flightModesZh(this);
}

QStringList Vehicle::flightModesZhForSimple()
{
    return _firmwarePlugin->flightModesZh(this, true);
}
...
QString Vehicle::flightModeZh() const
{
    return _firmwarePlugin->flightModeZh(_base_mode, _custom_mode);
}
...
void Vehicle::missionModeChangeAltitude(double altitudeChange)
{
    QGeoCoordinate coor(0, 0, altitudeChange);
    missionManager()->writeArduPilotGuidedMissionItem(coor, true);
}
...
int Vehicle::currentMissionSequence()
{
    return missionManager()->currentIndex();
}

int Vehicle::missionCount()
{
    return missionManager()->missionItems().count();
}

void Vehicle::setSpeed(int speed)
{
    // QUAD
    if (parameterManager()->parameterExists(defaultComponentId(), QString("WPNAV_SPEED"))) {
        try
        {
            parameterManager()->getParameter(defaultComponentId(), QString("WPNAV_SPEED"))->setRawValue(((float) speed * 100.0f));
        }
        catch(...)
        {
            qDebug() << "setParameter WPNAV_SPEED error!";
        }
    }
    else if (parameterManager()->parameterExists(defaultComponentId(), QString("WP_SPEED_MAX")))
    {
        try
        {
            parameterManager()->getParameter(defaultComponentId(), QString("WP_SPEED_MAX"))->setRawValue(((float) speed * 100.0f));
        }
        catch(...)
        {
            qDebug() << "setParameter WP_SPEED_MAX error!";
        }
    } // plane 3.7 and below with airspeed, uses ARSPD_ENABLE:
    else if ((parameterManager()->parameterExists(defaultComponentId(), QString("TRIM_ARSPD_CM")) &&
              parameterManager()->parameterExists(defaultComponentId(), QString("ARSPD_ENABLE"))
              && parameterManager()->parameterExists(defaultComponentId(), QString("ARSPD_USE")) &&
              parameterManager()->getParameter(defaultComponentId(), QString("ARSPD_ENABLE"))->rawValue().toFloat() == 1
              && parameterManager()->getParameter(defaultComponentId(), QString("ARSPD_USE"))->rawValue().toFloat() == 1) ||
             ...
             (parameterManager()->parameterExists(defaultComponentId(), QString("TRIM_ARSPD_CM")) &&
              parameterManager()->parameterExists(defaultComponentId(), QString("ARSPD_TYPE"))
              && parameterManager()->parameterExists(defaultComponentId(), QString("ARSPD_USE")) &&
              parameterManager()->getParameter(defaultComponentId(), QString("ARSPD_TYPE"))->rawValue().toFloat() > 0
              && parameterManager()->getParameter(defaultComponentId(), QString("ARSPD_USE"))->rawValue().toFloat() == 1))
    {
        try
        {
            parameterManager()->getParameter(defaultComponentId(), QString("TRIM_ARSPD_CM"))->setRawValue(QVariant(((float) speed * 100.0f)));
        }
        catch(...)
        {
            qDebug() << "setParameter TRIM_ARSPD_CM error!";
        }
    } // plane without airspeed
    else if (parameterManager()->parameterExists(defaultComponentId(), QString("TRIM_THROTTLE")) &&
             parameterManager()->parameterExists(defaultComponentId(), QString("ARSPD_USE"))
             && parameterManager()->getParameter(defaultComponentId(), QString("ARSPD_USE"))->rawValue().toFloat() == 0)
    {
        try
        {
            parameterManager()->getParameter(defaultComponentId(), QString("TRIM_THROTTLE"))->setRawValue(QVariant((float) speed));
        }
        catch(...)
        {
            qDebug() << "setParameter TRIM_THROTTLE error!";
        }
    }
}

void Vehicle::setLoiterRadius(int radius)
{
    if (parameterManager()->parameterExists(defaultComponentId(), QString("LOITER_RAD"))) {
        parameterManager()->getParameter(defaultComponentId(), QString("LOITER_RAD"))->setRawValue(QVariant(radius));
    }
    if (parameterManager()->parameterExists(defaultComponentId(), QString("WP_LOITER_RAD"))) {
        parameterManager()->getParameter(defaultComponentId(), QString("WP_LOITER_RAD"))->setRawValue(QVariant(radius));
    }
}

int Vehicle::loiterRadius()
{
    int radius = 0;
    if (parameterManager()->parameterExists(defaultComponentId(), QString("LOITER_RAD"))) {
        radius = parameterManager()->getParameter(defaultComponentId(), QString("LOITER_RAD"))->rawValue().toInt();
    }
    if (parameterManager()->parameterExists(defaultComponentId(), QString("WP_LOITER_RAD"))) {
        radius = parameterManager()->getParameter(defaultComponentId(), QString("WP_LOITER_RAD"))->rawValue().toInt();
    }
    return radius;
}
...
void Vehicle::_updateVehicleImageOpaque()
{
    QString vehicleImageOpaque;

    bool isActived = _toolbox->multiVehicleManager()->activeVehicle() == this;
    if (isCar()) {
        if (isActived) vehicleImageOpaque = _activeCarImage;
        else  vehicleImageOpaque = _carImage;
    } else {
        if (isActived) vehicleImageOpaque = _activePlaneImage;
        else vehicleImageOpaque = _planeImage;
    }
    if (vehicleImageOpaque.compare(_vehicleImageOpaque) != 0) {
        _vehicleImageOpaque = vehicleImageOpaque;
        emit vehicleImageOpaqueChanged();
    }

    QString vehiclePathColor;
    if (isActived) {
        vehiclePathColor = _activeColor;
    } else {
        vehiclePathColor = _color;
    }
    if (vehiclePathColor.compare(_vehiclePathColor) != 0) {
        _vehiclePathColor = vehiclePathColor;
        emit vehiclePathColorChanged();
    }
}
...
{
    if (!_vehicleImageOpaque.isEmpty()) {
        return _vehicleImageOpaque;
    } else {
        if(_firmwarePlugin)
...
QString Vehicle::vehiclePathColor() const
{
    return _vehiclePathColor;
}
...
void Vehicle::_handleNavControllerOutput(const mavlink_message_t &message)
{
    mavlink_nav_controller_output_t navControllerOutput;
    mavlink_msg_nav_controller_output_decode(&message, &navControllerOutput);
    _distanceToNextFact.setRawValue(navControllerOutput.wp_dist);
}
...
void Vehicle::goHome()
{
    //sendMavCommand(_defaultComponentId, MAV_CMD_DO_SET_SERVO, true, 16, 1900, 0, 0, 0, 0);

    QString parameName = "RC18_TRIM";
    if (parameterManager()->parameterExists(_defaultComponentId, parameName)) {
        parameterManager()->getParameter(_defaultComponentId, parameName)->setRawValue(1000);
    } else {
        qDebug() << __FILE__ << __func__ << __LINE__ << QString("parame name %1 not exists!").arg(parameName);
    }
}

void Vehicle::beiliGimbalMountControl(float pitch)
{
    float range_min = -1;
    float range_max = 1;
    int pwm_min = 1100;
    ...
    int pwm_dif = pwm_max - pwm_min;
    float pitch_m = (float)(range_max - range_min) / pwm_dif;

    uint16_t pitch_pwm = pitch;
    if (pitch == 0 || (pitch_m < 0.000001 && pitch_m > -0.000001)) pitch_pwm = pwm_min + pwm_dif / 2;
    else {
        pitch_pwm = pwm_min + (pitch - range_min) / pitch_m;
    }
    //qDebug() << "Gimbal:" << pitch_pwm;
    sendMavCommand(_defaultComponentId, MAV_CMD_DO_SET_SERVO, true, 9, pitch_pwm, 0, 0, 0, 0);
}

void Vehicle::setCameraStatus(int status)
{
    SharedLinkInterfacePtr  sharedLink = vehicleLinkManager()->primaryLink().lock();
    if (!sharedLink) {
        qCDebug(VehicleLog) << "_ackMavlinkLogData: primary link gone!";
    ...
    mavlink_camera_status_t cameraStatusMsg;
    cameraStatusMsg.time_usec = QDateTime::currentDateTime().currentMSecsSinceEpoch()*1000;
    cameraStatusMsg.target_system = _id;
    cameraStatusMsg.p1 = status;
    mavlink_message_t msg;
    mavlink_msg_camera_status_encode_chan(_mavlink->getSystemId(),
                                          _mavlink->getComponentId(),
                                          sharedLink->mavlinkChannel(),
                                          &msg,
                                          &cameraStatusMsg);
...
QList<QGeoCoordinate> Vehicle::missionCoordinate()
{
    QList<QGeoCoordinate> list;
    QList<MissionItem*> items = this->missionManager()->missionItems();
    for (int i = 0; i < items.count(); i++) {
        QGeoCoordinate coor = items.at(i)->coordinate();
        if (coor.isValid() && coor.latitude() != 0 && coor.longitude() != 0) {
            list.append(coor);
        }
    }
    return list;
