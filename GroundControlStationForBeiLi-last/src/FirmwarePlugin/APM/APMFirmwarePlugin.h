...
{
public:
    APMCustomMode(uint32_t mode, bool settable, bool isSimple);
    uint32_t modeAsInt() const { return _mode; }
    bool canBeSet() const { return _settable; }
    QString modeString() const;
    QString modeStringZh() const;
    bool isSimple() const { return _isSimple; }

protected:
    void setEnumToStringMapping(const QMap<uint32_t,QString>& enumToString);
    void setEnumToStringZhMapping(const QMap<uint32_t,QString>& enumToString);

private:
    uint32_t               _mode;
    bool                   _settable;
    QMap<uint32_t,QString> _enumToString;
    QMap<uint32_t,QString> _enumToStringZh; // QString 为中文
    bool                   _isSimple;   /// true:用于普通用户操作的基本模式
};
