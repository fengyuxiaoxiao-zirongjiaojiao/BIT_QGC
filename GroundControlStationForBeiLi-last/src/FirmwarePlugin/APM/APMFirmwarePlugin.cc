 ...
 */
APMCustomMode::APMCustomMode(uint32_t mode, bool settable, bool isSimple) :
    _mode(mode),
    _settable(settable),
    _isSimple(isSimple)
{
...
void APMCustomMode::setEnumToStringZhMapping(const QMap<uint32_t, QString> &enumToString)
{
     _enumToStringZh = enumToString;
}
...
QString APMCustomMode::modeStringZh() const
{
    QString mode = _enumToStringZh.value(modeAsInt());
    if (mode.isEmpty()) {
        mode = "mode" + QString::number(modeAsInt());
    }
    return mode;
...
QStringList APMFirmwarePlugin::flightModesZh(Vehicle *vehicle)
{
    Q_UNUSED(vehicle)
    QStringList flightModesList;
#ifdef Flytouav// 飞图垂起固定翼
    flightModesList << QStringLiteral("自动") << QStringLiteral("盘旋") << QStringLiteral("Q降落")
                    << QStringLiteral("Q悬停") << QStringLiteral("Q返航") << QStringLiteral("返航");

#else
    foreach (const APMCustomMode& customMode, _supportedModes) {
        if (customMode.canBeSet()) {
            flightModesList << customMode.modeStringZh();
        }
    }
#endif
...
QStringList APMFirmwarePlugin::flightModesZh(Vehicle *vehicle, bool isSimple)
{
    Q_UNUSED(vehicle)
    QStringList flightModesList;
    foreach (const APMCustomMode& customMode, _supportedModes) {
        if (customMode.canBeSet()) {
            if (isSimple) {
                if (customMode.isSimple()) flightModesList << customMode.modeStringZh();
            } else {
                flightModesList << customMode.modeStringZh();
            }
        }
...
QString APMFirmwarePlugin::flightModeZh(uint8_t base_mode, uint32_t custom_mode) const
{
    ...
    if (base_mode & MAV_MODE_FLAG_CUSTOM_MODE_ENABLED) {
        foreach (const APMCustomMode& customMode, _supportedModes) {
            if (customMode.modeAsInt() == custom_mode) {
                flightMode = customMode.modeStringZh();
            }
        }
    }
    return flightMode;
...
QStringList APMFirmwarePlugin::flightModes(Vehicle *vehicle, bool isSimple)
{
    Q_UNUSED(vehicle)
    QStringList flightModesList;
    foreach (const APMCustomMode& customMode, _supportedModes) {
        if (customMode.canBeSet()) {
            if (isSimple) {
                if (customMode.isSimple()) flightModesList << customMode.modeString();
            } else {
                flightModesList << customMode.modeString();
            }
        }
...
bool APMFirmwarePlugin::setFlightMode(const QString& flightMode, uint8_t* base_mode, uint32_t* custom_mode)
{
    ...
    *custom_mode = 0;
    ...
    foreach(const APMCustomMode& mode, _supportedModes) {
        if (flightMode.compare(mode.modeString(), Qt::CaseInsensitive) == 0 || flightMode.compare(mode.modeStringZh(), Qt::CaseInsensitive) == 0) {
